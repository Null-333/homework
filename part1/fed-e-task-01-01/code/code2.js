...
function averageDollarValue (cars) {
    const map = fp.map(function (car) {
        return car.dollar_value;
    });
    const compose = fp.flowRight(_average, map);
    return compose(cars);
}
...
function sanitizeNames(cars) {
    const getNames = fp.map((item) => item.name);
    const toLower = fp.map((item) => fp.toLower(item));
    const underscore = fp.map((item) => _underscore(item));
    const compose = fp.flowRight(underscore, toLower, getNames);
    return compose(cars);
}
// 2.打印测试
